      ...
      <div className='bg-white border border-b border-gray-200/50 backdrop-blur-[2px] py-4 px-7 sticky top-0 z-30'>
        <div className='container mx-auto flex items-center justify-between gap-5'>
          <div className='flex gap-5'>
            ...
            <Link to="/">
                <img src={Logo} alt="logo" className='h-[24px] md:h-[28px]' />
            </Link>
              ...
              return(
                <Link key={item.id} to={item.path}>
                  <li className='text-[15px] text-black font-medium list-none relative group cursor-pointer'>
                    {item.label}
                    <span
                      className={`absolute inset-x-0 bottom-0 h-[2px] bg-teal-300 transition-all duration-300 origin-left ${
          ...
          <div className='flex items-center gap-6'>
            ...
            ><LuSearch className='text-[22px]' /></button>

            {!user ? <button
              className='flex items-center justify-center gap-3 bg-linear-to-r from-teal-500 to-teal-300 text-xs md:text-sm font-semibold text-white px-5 md:px-7 py-2 rounded-full hover:bg-black hover:text-white transition-colors cursor-pointer hover:shadow-teal-300 hover:shadow-2xl'
              onClick={()=>setOpenAuthForm(true)}
            >Login/SignUp</button> : <div className='hidden md:block'>
              <ProfileInfoCard />
              </div>}
          </div>
